...
```text
pip install -e ".[test]" + python -m unittest
docker build -t agent-test:$GITHUB_SHA .
docker save agent-test:$GITHUB_SHA
sudo k3s ctr -n k8s.io images import /tmp/agent-test-$GITHUB_SHA.tar
kubectl apply -f k8s/test
kubectl -n test set image deployment/agent-test agent-test=agent-test:$GITHUB_SHA
kubectl -n test rollout status deployment/agent-test
```
...
kubectl -n prod set image deployment/agent-test agent-test=agent-test:$GITHUB_SHA
kubectl -n prod rollout status deployment/agent-test
```
...
- 不建议让 Argo CD 对这个应用开启 automated self-heal，否则可能把 Actions 设置的镜像 tag 回滚到 YAML 中的 `agent-test:local`。
