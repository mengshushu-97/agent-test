...
docker build -t harbor.local/test/agent-test:$GITHUB_SHA .
docker push harbor.local/test/agent-test:$GITHUB_SHA
...
```text
pip install -e ".[test]" + python -m unittest
docker build -t harbor.local/prod/agent-test:$GITHUB_SHA .
docker push harbor.local/prod/agent-test:$GITHUB_SHA
...
kubectl get applications -n argocd | grep agent-test
```
...
```bash
kubectl rollout undo deployment/agent-test -n test
kubectl rollout undo deployment/agent-test -n prod
```
